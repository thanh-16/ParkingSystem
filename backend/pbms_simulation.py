...
# Reconfigure stdout to use UTF-8 to print Vietnamese characters properly
...
state = {
    "ai_weights": {
        ...
        "w2": 0.3,  # Tầng thấp
        "w3": 0.2,  # Cân bằng tải
        "w4": 0.1   # Loại vé (Vé tháng đỗ xa)
    },
    "pricing_rules": {
        1: 5000.0,   # Motorbike
        2: 20000.0,  # Compact Car
        3: 30000.0,  # SUV
        4: 15000.0   # EV
    },
    "vehicle_types": {
...
def find_optimal_slot(vehicle_type_id, card_number):
    ...
    for slot in candidate_slots:
        # 1. Điểm khoảng cách (DistanceScore): Nhỏ hơn thì điểm cao hơn (ưu tiên gần)
        distance_score = 1.0 / max(1, slot["distance_metric"])
        
        # 2. Điểm tầng đỗ (FloorLevel): Tầng thấp hơn thì điểm cao hơn
        floor_level_score = 1.0 / max(1, abs(slot["floor"]))
        
        # 3. Điểm cân bằng tải (Utilization Balance): Tỷ lệ slot trống của tầng
        occ = occupancy_data[slot["floor"]]
        free_ratio = (occ["total"] - occ["occupied"]) / occ["total"]
        utilization_balance = free_ratio
        
        # 4. Điểm khớp thời gian (DurationMatch):
        # Vé tháng (dài hạn) ưu tiên đỗ xa, vé lượt ưu tiên đỗ gần
        if is_long_term:
            duration_match = slot["distance_metric"] / 100.0
        else:
            duration_match = 100.0 / max(1, slot["distance_metric"])

        # Hàm mục tiêu tối ưu đa tiêu chí (Multi-Criteria Objective Function)
...
def simulate_check_in():
    ...
    # Display Top 3 scored slots to show AI calculation
...
def simulate_check_out():
    ...
    # If Monthly Card, Fee is 0 (covered by monthly subscription)
...
def simulate_exception():
    ...
    session["check_out_time"] = datetime.now()
    session["notes"] = notes
    session["total_fee"] = 50000.0 # Standard lost ticket fine
