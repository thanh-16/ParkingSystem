...
namespace PBMS.AI.API.Consumers
{
    public class CheckInInitiatedEventConsumer : IConsumer<CheckInInitiatedEvent>
    {
        ...
        public async Task Consume(ConsumeContext<CheckInInitiatedEvent> context)
        {
            ...
            // 1. Try to pop optimal slot directly from Redis cache first (avoiding DB hits)
            ...
            if (optimalSlotId.HasValue)
            {
                ...
                // Set 5-Minute Cache Reservation Lock
                await _cache.ReserveSlotAsync(optimalSlotId.Value, TimeSpan.FromMinutes(5));

                // Update slot status in Registry DB to "Reserved" (2)
                try
                {
                ...
                // Publish SlotAllocatedEvent to finalize check-in
            ...
            // 2. Cache Fallback: Fetch available slots and floor occupancy snapshot from Registry microservice
            ...
            // 3. Multi-Criteria Objective Function Evaluation
            var (w1, w2, w3, w4) = await _cache.GetWeightsAsync();

            // For demo: Assume short-term by default, long-term if card starts with "MONTHLY"
            ...
            foreach (var slot in availableSlots)
            {
                ...
                // Utilization Balance (Actual free ratio based on full floor occupancy snapshot)
                ...
                if (occupancyList != null)
                {
                    ...
                    {
                        // Free ratio = (Total - Occupied) / Total
            ...
            // 4. Pop optimal slot from Redis after repopulating
            ...
            if (optimalSlotId.HasValue)
            {
                ...
                // Update slot status in Registry DB to "Reserved"
                try
                {
                    var updateUrl = $"{registryBaseUrl.TrimEnd('/')}/api/v1/registry/slots/update-status";
                    // Status 2 is "Reserved"
                    var response = await client.PostAsJsonAsync(updateUrl, new { SlotId = optimalSlotId.Value, Status = 2 });
                    if (!response.IsSuccessStatusCode)
                    {
                        Console.WriteLine("Failed to update slot status in Registry DB.");
