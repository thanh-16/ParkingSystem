...
namespace PBMS.Registry.API.Controllers
{
    ...
    public class RegistryController : ControllerBase
    {
        ...
        {
            // Logical Data Partitioning: isolate slots based on vehicle type
        ...
        public async Task<IActionResult> UpdateSlotStatus([FromBody] UpdateSlotStatusRequest request)
        {
            ...
            try
            {
                ...
                // Synchronize Redis Cache of AI Service
                if (request.Status != SlotStatus.Available && oldStatus == SlotStatus.Available)
                {
                    // Slot is no longer available. Remove it from Redis cache.
                ...
                {
                    // Slot is now available. Restore it in Redis cache.
        ...
        public async Task<IActionResult> SetupRegistryData()
        {
            ...
            // Seed Floors
            var floorB1 = new Floor { FloorNumber = -1, AllowedVehicleTypeId = 1, TotalSlots = 20 }; // Motorbikes
            var floor1 = new Floor { FloorNumber = 1, AllowedVehicleTypeId = 2, TotalSlots = 15 };  // Compact cars
            var floor2 = new Floor { FloorNumber = 2, AllowedVehicleTypeId = 3, TotalSlots = 10 };  // SUVs
            var floor3 = new Floor { FloorNumber = 3, AllowedVehicleTypeId = 4, TotalSlots = 10 };  // EVs
        ...
        public async Task<IActionResult> DeleteSlot(Guid id)
        {
            ...
            // Publish SlotDeletedEvent for Redis sync in AI Service
            await _publishEndpoint.Publish(new SlotDeletedEvent
            {
                SlotId = slot.Id,
                VehicleTypeId = vehicleTypeId
