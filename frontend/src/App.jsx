...
// Pre-seeded slots data for offline simulation mode
const INITIAL_SLOTS = [
  // Basement (Floor -1) - Allowed vehicle type = 1 (Motorbikes), total 20 slots
  ...Array.from({ length: 20 }, (_, i) => ({
    id: `slot-b1-${i + 1}`,
    slotNumber: `B1-M${(i + 1).toString().padStart(2, '0')}`,
    floorNumber: -1,
    allowedVehicleTypeId: 1, // Motorbike
    distanceMetric: (i + 1) * 3,
    status: 'Available',
    occupiedBy: null,
    bookingId: null
  })),
  // Floor 1 (F1) - Allowed vehicle type = 2 (Compact/Sedan), total 15 slots
  ...Array.from({ length: 15 }, (_, i) => ({
    id: `slot-f1-${i + 1}`,
    slotNumber: `F1-C${(i + 1).toString().padStart(2, '0')}`,
    floorNumber: 1,
    allowedVehicleTypeId: 2, // Sedan
    distanceMetric: (i + 1) * 4,
    status: i === 2 ? 'Occupied' : 'Available', // Seed one occupied
    occupiedBy: i === 2 ? '30A-12345' : null,
    bookingId: null
  })),
  // Floor 2 (F2) - Allowed vehicle type = 3 (SUVs), total 10 slots
  ...Array.from({ length: 10 }, (_, i) => ({
    id: `slot-f2-${i + 1}`,
    slotNumber: `F2-S${(i + 1).toString().padStart(2, '0')}`,
    floorNumber: 2,
    allowedVehicleTypeId: 3, // SUV
    distanceMetric: (i + 1) * 5,
    status: i === 1 ? 'Occupied' : 'Available', // Seed one occupied
    occupiedBy: i === 1 ? '29C-56789' : null,
    bookingId: null
  })),
  // Floor 3 (F3) - Allowed vehicle type = 4 (EVs), total 10 slots
  ...Array.from({ length: 10 }, (_, i) => ({
    id: `slot-f3-${i + 1}`,
    slotNumber: `F3-E${(i + 1).toString().padStart(2, '0')}`,
    floorNumber: 3,
    allowedVehicleTypeId: 4, // EV
...
const INITIAL_SESSIONS = [
  {
    ...
    slotNumber: 'F1-C03',
    cardNumber: 'CARD-101',
    checkInTime: new Date(Date.now() - 3.5 * 60 * 60 * 1000).toISOString(), // 3.5 hrs ago
    checkOutTime: null,
    status: 'Active',
    notes: null,
    totalFee: 0
  },
  {
    ...
    slotNumber: 'F2-S02',
    cardNumber: 'CARD-102',
    checkInTime: new Date(Date.now() - 1.2 * 60 * 60 * 1000).toISOString(), // 1.2 hrs ago
    checkOutTime: null,
    status: 'Active',
    notes: null,
    totalFee: 0
...
function App() {
  ...
  const [mode, setMode] = useState('offline'); // 'offline' (Simulation) or 'live' (API Gateway)
  ...
  const [revenue, setRevenue] = useState(40000); // Seeded starting revenue
  ...
  const [showExceptionModal, setShowExceptionModal] = useState(null); // active session object
  const [exceptionNotes, setExceptionNotes] = useState('');
  const [exceptionFee, setExceptionFee] = useState(0);

  // Driver App View States
  const [driverActiveTab, setDriverActiveTab] = useState('book'); // 'book' or 'track'
  ...
  // Auto Check-In & LED Guidance States
  const [autoCheckIn, setAutoCheckIn] = useState(true);
  const [ledScreenData, setLedScreenData] = useState(null);

  // Listen to external ANPR Camera Simulator messages (from camera_simulator.html)
  useEffect(() => {
    ...
    channel.onmessage = (event) => {
      ...
      // Automatically run AI check-in if checked
      if (autoCheckIn) {
        setTimeout(() => {
  ...
  // Setup periodic live status check and sync
  useEffect(() => {
    let interval;
    if (mode === 'live') {
      const checkLiveStatus = async () => {
        try {
          ...
          if (res.ok) {
            setApiOnline(true);
            // Sync weights & rules if connected
            syncLiveConfig();
          } else {
            setApiOnline(false);
          }
  ...
  // Toggle Slot Maintenance (Block / Unblock)
  const handleToggleMaintenance = async (slot, shouldBlock) => {
    const newStatusInt = shouldBlock ? 3 : 0; // 3: Maintenance, 0: Available
  ...
  // AI Scoring Engine (Offline Logic)
  const calculateOfflineAIScores = (vehicleTypeId, isMonthly) => {
    // 1. Filter slots supporting this vehicle type
    ...
    const results = candidateSlots.map((slot) => {
      ...
      // Floor Score: Ground (1) = 100, Basement (-1) = 80, Level 2 = 60, Level 3 = 40
      let floorScore = 100;
      ...
      // Utilization balance score: prefer floor with lower occupancy (higher free slots ratio)
      ...
      // Duration Match score: monthly subscription parked further, visitor parked closer
      let durScore = 0;
      if (isMonthly) {
        // Preferred further slots
        durScore = Math.round((slot.distanceMetric / 100) * 100);
        if (durScore > 100) durScore = 100;
      } else {
        // Preferred closer slots
  ...
  const runAICheckInAnalysis = () => {
    ...
    // Enforce B1 Motorbike capacity limit for manual flow
  ...
  const confirmCheckIn = async () => {
    ...
    // Handle booking resolution if pre-booked
  ...
  const triggerAutoCheckIn = async (customPlate = null, customVehicleTypeId = null) => {
    ...
    // Enforce B1 Motorbike capacity limit
    if (vTypeId === 1) {
      const occupiedMotorbikes = slots.filter(s => s.allowedVehicleTypeId === 1 && s.status === 'Occupied').length;
      if (occupiedMotorbikes >= 20) {
        addToast('Cảnh báo: Hầm xe máy B1 đã đầy chỗ (20/20)! Hệ thống dừng nhận xe máy.', 'error');
    ...
    // 2. If no pre-booking, run AI scoring
  ...
  const simulateIncomingCar = () => {
    ...
    const randomVehicleTypeId = Math.floor(Math.random() * 4) + 1; // 1 to 4
  ...
  const searchCheckOutSession = () => {
    ...
    const durationHours = Math.ceil(durationMs / (1000 * 60 * 60)); // Round up hourly fee
  ...
  const handleDriverBooking = async (e) => {
    ...
    // Run scoring algorithm for driver
    const scores = calculateOfflineAIScores(
      parseInt(driverBookingForm.vehicleTypeId),
      false // default casual visitor when prebooking
    );
  ...
  // Helper quick actions to autofill checkout
      ...
      {user && (
        <div className="app-layout">
          ...
          <main className="workspace">
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                  ...
                  <div>
                    ...
                    <p style={{ margin: '4px 0 0 0', fontSize: '12.5px', color: '#991b1b', lineHeight: '1.4' }}>
                      Cổng API Gateway (<code>http://localhost:5125</code>) hiện không thể kết nối. Vui lòng đảm bảo các microservices đã được khởi chạy bằng lệnh: <code>cd backend && docker-compose up -d</code>. Bạn cũng có thể bấm nút bên cạnh để chuyển sang chế độ mô phỏng offline ngay lập tức.
                    </p>
                  </div>
                </div>
                <button
            ...
            {/* TAB CONTENT 1: MANAGER DASHBOARD */}
            {activeTab === 'dashboard' && (
              <>
                {/* Stats Cards */}
            ...
            {activeTab === 'gates' && (
              <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
                ...
                {/* Finished Exception Sessions list */}
              ...
              <div style={{ display: 'flex', gap: '40px', justifyContent: 'center', alignItems: 'flex-start' }}>
                ...
                <div className="phone-mockup">
                  ...
                  <div className="phone-content">
                    ...
                    {/* App Tab 2: Track active session */}
        ...
        <div className="overlay-dialog">
          <div className="dialog-content">
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '12px', fontSize: '13.5px' }}>
              ...
              {/* Action Buttons for Manager & Staff */}
